...
input_tensor,input_lang,target_tensor,target_lang= read_data(os.getcwd() + gConfig['seq_data1k'], gConfig['max_train_data_size'])
hidden_size = 256
...
def train():
    ...
    checkpoint_dir = os.getcwd() + gConfig['model_data']

    checkpoint_prefix = os.path.join(checkpoint_dir, "model_data.pt")
    ...
    if os.path.exists(checkpoint_prefix) and os.path.getsize(checkpoint_prefix) > 0:
    ...
    while batch_loss>gConfig['min_loss']:
        ...
        for i in range(1,(max_data//BATCH_SIZE)):
            ...
            print('the number of train step:{} the time of the train step loss {:.4f}'.format(i,batch_loss ))
        step_time_epoch = (time.time() - start_time_epoch) / steps_per_epoch
        step_loss = total_loss / steps_per_epoch
        current_steps = +steps_per_epoch
        step_time_total = (time.time() - start_time) / current_steps
        print('the amount of train step: {} the time of the train step: {}  the new time of the train step: {} the new every train step loss {:.4f}'.format(current_steps, step_time_total, step_time_epoch,
                                                                      batch_loss))
